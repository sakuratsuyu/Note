...
??? proof
    ...
    Since $g(t)$ has $n + 2$ distinct roots $x, x_0, \dots, x_n$, by Generalized Rolle's Theorem,
...
??? example
    ...
    **Solution.**
    ...
    Thus let $\frac{eh^2}{8} \le 10^{-6}$, we have $h \le 1.72 \times 10^{-3}$. To make $N = (1 - 0) / h$ an integer, we can simply choose $h = 0.001$.
...
then from the theorem above, the interpolating polynomials can be generated **recursively**. 
...
$$
    N(x) = f[x_0] + \sum\limits_{k = 1}^n (-1)^k \binom{-s}{k} \nabla^k f(x_0).
$$
...
!!! theorem "Theorem 3.6"
    If $f \in C^1[a, b]$ and $x_0, \dots, x_n \in [a, b]$ are distinct, the unique polynomial of least degree agreeing with $f$ and $f'$ at $x_0, \dots, x_n$ is the **Hermit Polynomial** of degree *at most* **2n + 1** defined by
...
Based on the **Theorem 3.5**, we redefine that 
...
In nature, it's **piecewise cubic Hermite polynomial**, and the curve is call **Bezier curve**.
