...
!!! theorem "Corollary"
    ...
    $\Rightarrow$ The smaller the $k$ is, the faster it converges.
...
It is still convegent, but <u>not quadratically</u>.
...
<div align="center">
	<img src="../Pic/chap2_1.png" alt="chap2_1 " style="width:600px"/>
</div>
...
!!! theorem
    Suppose that $\{p_n\}$ is a sequence, $\lim_{n\rightarrow \infty}p_n = p$,$\exists N$, s.t. $\forall n > N, (p_n - p)(p_{n+1} -p) \gt 0$. Then the sequence $\{\hat{p_n}\}$ converges to $p$ **faster** than $\{p_n\}$ in the sense that
